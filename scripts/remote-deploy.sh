...
DEPLOY_PATH="${DEPLOY_PATH:-/opt/beterahoy.es}"
SERVICE_NAME="${SERVICE_NAME:-beterahoy}"
...
export DATABASE_URL="${DATABASE_URL:-postgresql://beterahoy:beterahoy@127.0.0.1:5437/beterahoy}"
export NEXT_PUBLIC_SITE_URL="${NEXT_PUBLIC_SITE_URL:-https://www.beterahoy.es}"
# NextAuth: NEXTAUTH_URL y NEXTAUTH_SECRET en .env (ve .env.example)

# Mismo mapeo que docker-compose: host 127.0.0.1:5437 -> contenedor 5432
if [ -f docker-compose.yml ] && echo "$DATABASE_URL" | grep -qE ':(5434|5435)/'; then
  echo "ERROR: DATABASE_URL usa puerto 5434/5435 (lelianahoy/sermestre). beterahoy.es usa 127.0.0.1:5437 (ver docker-compose.yml y .env)." >&2
  exit 1
fi
...
if [ -f docker-compose.yml ]; then
  ...
  for i in $(seq 1 90); do
    ...
    if [ "$i" -ge 90 ]; then
      echo "ERROR: timeout esperando PostgreSQL en el contenedor beterahoy-db" >&2
      exit 1
    fi
    sleep 1
  done

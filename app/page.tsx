...
export const metadata: Metadata = {
  ...
  openGraph: {
    type: "website",
    title: `${SITE_NAME} | Noticias e información de Bétera`,
      ...
      <div className="container-page space-y-12 py-8 md:py-10">
        ...
        <div className="grid gap-10 lg:grid-cols-3 lg:gap-8">
          <div className="min-w-0 space-y-8 lg:col-span-2">
            <section>
              <SectionHeader
                title={isVal ? "L'últim a Bétera" : "Lo último en Bétera"}
                icon="📰"
                subtitle={
                  isVal
              ...
              <div>
                {latestArticles.length === 0 ? (
                  ...
                  <>
                    ...
                    <div className="mt-6 space-y-2 divide-y divide-betera-sand/80">
                      {restArticles.map((a) => (
            ...
            <section>
              <SectionHeader
                title={isVal ? "Esta setmana a Bétera…" : "Esta semana en Bétera…"}
                icon="📅"
                    ...
                    {thisWeekEvents.slice(0, 4).map((eventItem) => {
                      ...
                      return (
                        <article key={eventItem.id} className={ui.card}>
                          {thumb ? (
                            <Link href={`/eventos/${eventItem.slug}`} className="block overflow-hidden rounded-xl border border-betera-sand">
                          ...
                          <div className="mt-3 flex flex-wrap items-center gap-1.5">
                            <span className="rounded-md border border-betera-sand bg-betera-mist px-2 py-0.5 text-[11px] font-semibold text-betera-forest">
                          ...
                          <h3 className="mt-2 line-clamp-2 font-serif text-base font-semibold text-betera-ink">
                            <Link href={`/eventos/${eventItem.slug}`} className="hover:text-betera-leaf">
                              {titleText}
                            </Link>
                          </h3>
                ...
                {latestReports.length === 0 ? (
                  ...
                  latestReports.map((report) => (
                    <article key={report.id} className={ui.card}>
                      {report.imageUrl ? (
                        <div className="mb-3 overflow-hidden rounded-xl border border-betera-sand">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5">
                        {report.categories.slice(0, 3).map((tag) => (
                          <Link
                            ...
                            className="rounded-md border border-betera-sand bg-betera-mist px-2 py-0.5 text-[11px] font-semibold text-betera-forest hover:bg-betera-sand/50"
                          >
                            #{tag}
                          </Link>
                        ))}
                      </div>
                      <h3 className="mt-2 line-clamp-2 font-serif text-base font-semibold text-betera-ink">{report.title}</h3>
                      <p className="mt-1 line-clamp-3 text-sm text-betera-ink/75">{truncateWords(stripMarkdownToPlain(report.content), 24)}</p>
                      <div className="mt-3 flex items-center justify-between">
          ...
          <aside className="space-y-8 lg:col-span-1">
            ...
            <div className={`${ui.card} !p-3`}>
              <CtaLink
                href="/elecciones-municipales-betera-2027"
                trackParams={{
                  cta_name: "home_elecciones_2027_sidebar",
                  cta_context: "home_sidebar",
                  destination: "/elecciones-municipales-betera-2027",
                }}
                className="block overflow-hidden rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-betera-leaf focus-visible:ring-offset-2"
              >
                ...
                <img
                  ...
                  alt={
                    isVal
                      ...
                      : "Especial Elecciones Municipales 2027 en Bétera – ir a la información"
                  }
            ...
            <div className="rounded-2xl border-2 border-dashed border-betera-leaf/40 bg-betera-forest/5 p-4 text-sm text-betera-forest">
              <p className="font-bold">{isVal ? "Tens alguna cosa que contar o denunciar?" : "¿Algo que contar o denunciar?"}</p>
              <p className="mt-1 text-betera-ink/75">{isVal ? "Passa per denúncies: ho revisem abans de publicar." : "Pasa por denuncias: lo revisamos antes de publicar."}</p>
              <CtaLink
        ...
        <section>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {highlightedPages.map((item) => (
              ...
              >
                <span className="font-semibold text-betera-ink group-hover:text-betera-leaf">
                  {localizedText(locale, item.title, item.titleVal)}
                </span>
                <span className={`mt-1 block text-xs ${ui.muted}`}>
                  {isVal ? "Guia pràctica a Bétera" : "Guía práctica en Bétera"}
                </span>
              </Link>
            ))}
          </div>

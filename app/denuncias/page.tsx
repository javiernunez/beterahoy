...
export const metadata: Metadata = {
  title: "Denuncias ciudadanas de Bétera",
  description: `Incidencias vecinales publicadas en Bétera: ruido, tráfico, limpieza y más. Voz de barrio y seguimiento en ${SITE_NAME}, Camp de Túria.`,
  alternates: { canonical: pageUrl },
  openGraph: {
    title: `Denuncias vecinales en Bétera | ${SITE_NAME}`,
  ...
  return (
    <div className="container-page">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link href="/denuncias/nueva" className="rounded bg-betera-leaf px-4 py-2 text-sm font-semibold text-white">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Link
          href="/denuncias"
          className={`rounded-full border px-3 py-1 text-xs font-medium ${activeTag ? "border-slate-300 text-slate-700" : "border-blue-600 bg-betera-leaf text-white"}`}
        >
          {isVal ? "Totes" : "Todas"}
        </Link>
        {REPORT_CATEGORIES.map((category) => {
          ...
          return (
            <Link
              ...
              className={`rounded-full border px-3 py-1 text-xs font-medium ${isActive ? "border-blue-600 bg-betera-leaf text-white" : "border-slate-300 text-slate-700"}`}
            >
              #{category}
            </Link>
          );

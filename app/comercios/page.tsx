...
export const metadata: Metadata = {
  ...
  description: "Explora secciones de comercios de Bétera y descubre 12 negocios destacados.",
  ...
  return (
    <div className="container-page max-w-6xl space-y-8 py-8 md:py-10">
      <section>
        ...
        />
        <aside className="mt-6 rounded-2xl border border-betera-leaf/30 bg-betera-mist/70 px-4 py-3 text-sm text-betera-forest">
          <p className="font-semibold">{isVal ? "La teva opinió compta" : "Tu opinión cuenta"}</p>
          <p className="mt-1 text-betera-leaf-dark/95">
            {isVal
        ...
        <nav className="flex flex-wrap items-center gap-2" aria-label={isVal ? "Submenú de comerços" : "Submenú de comercios"}>
          {COMMERCE_SECTIONS.map((section) => (
            <Link
              ...
              className="group flex items-center gap-1.5 rounded-full border border-slate-300 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-blue-400 hover:bg-white hover:text-betera-leaf-dark hover:shadow-sm"
            >
              ...
              <article key={business.id} className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
                ...
                <div className="p-4">
                  ...
                  <h3 className="mt-1 text-base font-semibold text-slate-900">
                    <Link href={`/comercios/${business.slug}`} className="hover:text-betera-leaf-dark hover:underline">
                      {localizedName}
                    </Link>
                  </h3>
                  ...
                  <Link
                    ...
                    className="mt-2 inline-block text-sm font-semibold text-betera-leaf hover:underline"
                  >

...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const localKw =
    item.kind === "ASSOCIATION" || item.kind === "SPORT"
      ? (["asociación Bétera", "asociaciones Bétera", "voluntariado Bétera"] as const)
      : item.kind === "POLITICS"
        ? (["política Bétera", "partidos Bétera"] as const)
        : (["comercio Bétera", "comercios Bétera", "negocios Bétera"] as const);
  ...
  return (
    <div className="container-page max-w-5xl space-y-8 py-8 md:py-10">
      ...
      <section className="space-y-4">
        <Link href="/comercios" className="text-sm font-semibold text-betera-leaf hover:underline">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="p-5">
            ...
            {item.websiteUrl ? (
              <a href={item.websiteUrl} target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm font-semibold text-betera-leaf hover:underline">
                {isVal ? "Web oficial →" : "Web oficial →"}
              </a>
            ) : item.href ? (
              <a href={item.href} target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm font-semibold text-betera-leaf hover:underline">
      ...
      <RelatedLinksSection
        ...
        subtitle={isVal ? "Altres negocis del mateix tipus a Bétera." : "Otros negocios del mismo tipo en Bétera."}

...
export const metadata: Metadata = {
  title: "Información útil para vecinos de Bétera",
  description:
    "Guías prácticas: colegios, salud, trámites, transporte y recursos esenciales para vivir en Bétera, Camp de Túria, Valencia.",
  alternates: { canonical: pageUrl },
  openGraph: {
    title: `Información útil y guías en Bétera | ${SITE_NAME}`,
    ...
    <div className="container-page">
      ...
      <div className="mb-10 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 p-6 text-white shadow-lg md:p-10">
        ...
        <p className="mt-2 max-w-2xl text-blue-100 md:text-lg">
          {isVal
            ? "Tot el que necessites saber per viure a Bétera: col·legis, salut, tràmits i transport, organitzats perquè ho trobes ràpid."
            : "Todo lo que necesitas saber para vivir en Bétera: colegios, salud, trámites y transporte, organizados para que lo encuentres rápido."}
        </p>
      </div>
            ...
            {highlighted.map((page) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-betera-mist text-xs">⭐</span>
                  <h3 className="font-semibold text-slate-900 group-hover:text-betera-leaf line-clamp-2 text-sm">
                    {page.title}
                  </h3>
                </div>
              </Link>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {uncategorized.map((page) => (
              <Link
                ...
                className="group rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-400 hover:shadow"
              >
                <h3 className="font-semibold text-slate-900 group-hover:text-betera-leaf text-sm">
                  {page.title}
                </h3>
                {page.isHighlighted ? (
                  <span className="mt-1 inline-block rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-betera-leaf">

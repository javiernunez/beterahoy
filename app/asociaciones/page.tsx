...
export const metadata: Metadata = {
  title: "Asociaciones",
  description: "Tejido asociativo de Bétera: casales, clubes deportivos, AMPAs, asociaciones vecinales y ONGs.",
  ...
  return (
    <div className="container-page max-w-6xl space-y-8 py-8 md:py-10">
      <SectionHeader
        ...
        subtitle={isVal ? "Entitats i teixit social de Bétera." : "Entidades y tejido social de Bétera."}
      />
              ...
              <article key={entry.id} className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
                ...
                <div className="p-4">
                  ...
                  <h3 className="mt-1 text-base font-semibold text-slate-900">
                    <Link href={`/asociaciones/${entry.slug}`} className="hover:text-betera-leaf-dark hover:underline">
                      {name}
                    </Link>
                  </h3>
                  <p className="mt-2 text-sm leading-relaxed text-slate-600">{truncateWords(stripMarkdownToPlain(description), 22)}</p>
                  <Link href={`/asociaciones/${entry.slug}`} className="mt-2 inline-block text-sm font-semibold text-betera-leaf hover:underline">

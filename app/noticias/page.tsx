...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const description = label
    ? `Noticias de Bétera: categoría ${label}. Actualidad y sucesos locales, Camp de Túria, Valencia.`
    : "Qué pasa en Bétera: actualidad, política local, sucesos, cultura y deporte. Noticias de la Camp de Túria, Comunitat Valenciana.";

  return {
    title: label ? `Noticias: ${label}` : "Noticias de Bétera",
    description,
    alternates: { canonical: pageUrl },
    openGraph: {
      type: "website",
      title: label ? `Noticias: ${label} | ${SITE_NAME}` : `Noticias de Bétera | ${SITE_NAME}`,
      description,
      url: pageUrl,
      locale: "es_ES",
      siteName: SITE_NAME,
...
export default async function NewsPage({ searchParams }: Readonly<Props>) {
  ...
  return (
    <PageShell
      ...
      subtitle={
        isVal
          ? "Actualitat i successos a Bétera, Camp de Túria. Filtra per temàtica o recorre la cronologia."
          : "Actualidad y sucesos en Bétera, Camp de Túria. Filtra por temática o recorre la cronología."
      }
    >
      ...
      <div className="mt-8">
        {articles.length === 0 ? (
          ...
          <>
            ...
            <div className="mt-6 divide-y divide-betera-sand">
              {rest.map((article) => (
      ...
      <section className="mt-10 grid gap-3 rounded-2xl border border-betera-leaf/30 bg-betera-mist/60 p-4 md:grid-cols-2">
        <div>
          <p className="text-sm font-semibold text-betera-forest">{isVal ? "Has vist alguna incidència al teu barri?" : "¿Has visto alguna incidencia en tu barrio?"}</p>
          <p className="mt-1 text-sm text-betera-leaf-dark/90">
...
function FilterLink({ href, active, label }: Readonly<{ href: string; active: boolean; label: string }>) {
  return (
    <Link
      ...
      className={
        active
          ? "inline-flex items-center rounded-lg border-2 border-betera-forest bg-betera-forest/10 px-3 py-1.5 text-sm font-bold text-betera-forest"
          : "inline-flex items-center rounded-lg border border-betera-sand bg-white px-3 py-1.5 text-sm font-medium text-betera-ink/80 hover:border-betera-leaf/40 hover:bg-betera-mist"
      }
    >
      {label}
    </Link>

...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${school.name} — Colegio ${typeLabel} en Bétera`,
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
      ...
      <div className="mt-6 space-y-3">
        ...
        {school.phone ? (
          <div className="flex items-start gap-3 text-slate-700">
            <span className="mt-0.5 text-lg" aria-hidden>📞</span>
            <a href={`tel:${school.phone.replace(/\s/g, "")}`} className="font-semibold text-betera-leaf hover:underline">
              {school.phone}
            </a>
          </div>
        ) : null}
        {school.website ? (
          <div className="flex items-start gap-3 text-slate-700">
            <span className="mt-0.5 text-lg" aria-hidden>🌐</span>
            <a href={school.website} target="_blank" rel="noopener noreferrer" className="break-all font-medium text-betera-leaf underline decoration-blue-400/80 underline-offset-2 hover:text-betera-forest">
  ...
  return (
    <div className="container-page max-w-3xl">
      ...
      <div className="mb-6">
        <Link
          href="/colegios"
          className="mb-3 inline-flex items-center gap-1 text-sm font-medium text-slate-500 transition-colors hover:text-betera-leaf"
        >
      ...
      <section className="mt-10">
        <h2 className="mb-1 text-lg font-bold text-slate-900">
          {isVal ? "Altres col·legis a Bétera" : "Otros colegios en Bétera"}
        </h2>
          ...
          {otherSchools.map((s) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <h3 className="text-sm font-semibold text-slate-900 group-hover:text-betera-leaf">{s.name}</h3>

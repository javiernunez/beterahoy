...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const description = label
    ? `Vídeos de Bétera, categoría ${label}.`
    : "Videos de Bétera: entrevistas, reportajes y contenido local.";
  ...
  return {
    ...
    openGraph: {
      type: "website",
      title: label ? `Videos: ${label} | ${SITE_NAME}` : `Videos de Bétera | ${SITE_NAME}`,
      description,
      url: pageUrl,
      locale: "es_ES",
      siteName: SITE_NAME,
...
function FilterLink({ href, active, label }: { href: string; active: boolean; label: string }) {
  return (
    <Link
      ...
      className={
        active
          ? "inline-flex items-center rounded-full border-2 border-blue-600 bg-betera-mist px-3 py-1.5 text-sm font-semibold text-betera-forest"

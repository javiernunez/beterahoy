...
export const metadata: Metadata = {
  title: "Eventos y agenda en Bétera",
  description:
    "Agenda de eventos, cultura, deporte y fiestas en Bétera, Camp de Túria. Consulta los próximos actos y entra al detalle de cada evento.",
  alternates: { canonical: pageUrl },
  openGraph: {
    title: `Eventos y cultura en Bétera | ${SITE_NAME}`,
    description: "Consulta el calendario de actividades en Bétera y entra al detalle de cada evento.",
    url: pageUrl,
    type: "website",
    locale: "es_ES",
    siteName: SITE_NAME,
      ...
      <Link href={`/eventos/${eventItem.slug}`} className="block">
        <div className="relative h-40 w-full bg-slate-100">
          {eventImage ? (
            ...
            <div className="flex h-full items-center justify-center bg-gradient-to-br from-blue-100 to-cyan-100 text-betera-leaf-dark">
              <span className="px-4 text-center text-sm font-semibold">{isVal ? "Pla a Bétera" : "Plan en Bétera"}</span>
            </div>
          )}
        </div>
      </Link>
      <div className="space-y-2 p-4">
        <div className="flex flex-wrap items-center gap-2">
          {eventItem.category === "generico" ? null : (
            <span className="inline-block rounded-full bg-betera-mist px-2 py-0.5 text-[11px] font-semibold text-betera-forest">
        ...
        <Link
          ...
          className="line-clamp-2 text-base font-semibold text-slate-900 group-hover:text-betera-leaf-dark group-hover:underline"
        >
          {eventTitle}
        </Link>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Link
              href={`/eventos/${eventItem.slug}`}
              className="text-xs font-semibold text-betera-leaf hover:text-betera-leaf-dark hover:underline"
            >
    ...
    <div className="container-page max-w-5xl">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
        ...
        <h1 className="mt-2 text-3xl font-bold text-slate-900 md:text-4xl">
          {isVal ? "Esdeveniments a Bétera" : "Eventos en Bétera"}
        </h1>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600 md:text-base">
          {isVal
            ? "Si no saps què fer hui a Bétera o els pròxims dies, ací tens plans de cultura, esport, festes i activitats per a gaudir del poble."
            : "Si no sabes qué hacer hoy en Bétera o los próximos días, aquí tienes planes de cultura, deporte, fiestas y actividades para disfrutar del pueblo."}
        </p>
          ...
          <Link
            ...
            className={`rounded-full border px-3 py-1 text-xs font-semibold transition-colors ${
              selectedCategory === null
                ? "border-betera-leaf/40 bg-betera-mist text-betera-forest"
      ...
      <section className="mt-10 rounded-2xl border border-slate-200 bg-white p-5 text-sm leading-6 text-slate-700 md:p-6">
        <h2 className="text-lg font-semibold text-slate-900">
          {isVal ? "Què fer hui a Bétera" : "Qué hacer hoy en Bétera"}
        </h2>
        <p className="mt-2">
          {isVal
            ? "Esta agenda s'actualitza de manera contínua per a ajudar-te a trobar idees de què fer hui a Bétera i també durant els pròxims dies: teatre, música, activitats familiars, esports, festes i propostes culturals."
            : "Esta agenda se actualiza de forma continua para ayudarte a encontrar ideas de qué hacer hoy en Bétera y también durante los próximos días: teatro, música, actividades familiares, deporte, fiestas y propuestas culturales."}
        </p>
        <div className="mt-4 flex flex-wrap gap-3">
          <CtaLink
            ...
            className="rounded-lg bg-blue-700 px-3 py-2 text-sm font-semibold text-white hover:bg-betera-leaf"
          >

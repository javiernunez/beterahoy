    ...
    <section className="mt-12 border-t border-slate-200 pt-8" aria-labelledby="article-comments-heading">
      ...
      {authRequired ? (
        <p className="mt-2 text-sm text-slate-700">
          {t.authHint}{" "}
          <Link href={loginHref} className="font-semibold text-betera-leaf hover:underline">
            {t.login}
          </Link>
          {" · "}
          <Link href={registerHref} className="font-semibold text-betera-leaf hover:underline">
            {t.register}
          </Link>
        </p>
      ) : null}

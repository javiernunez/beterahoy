...
export function CommerceReviewForm({ entryId }: Props) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      {notice ? <p className="text-sm text-betera-leaf">{notice}</p> : null}
      ...
      <button disabled={busy} type="submit" className="rounded bg-betera-leaf px-4 py-2 text-sm font-semibold text-white">

...
export function NewsCard({ article, compact = false, lead = false }: Props) {
  ...
  if (lead) {
    return (
      <article className="overflow-hidden rounded-2xl border border-betera-sand/90 bg-white shadow-betera">
        {img ? (
        ...
        <div className="p-5 md:p-6">
          <div className="mb-3 flex flex-wrap items-center gap-2 text-sm text-betera-ink/55">
            <CategoryChip category={article.category} />
            <time dateTime={article.createdAt.toISOString()}>{formatDate(article.createdAt)}</time>
          </div>
          <h2 className="font-serif text-2xl font-bold leading-tight text-betera-ink md:text-3xl lg:text-[2rem]">
            <Link href={`/noticias/${article.slug}`} className="transition hover:text-betera-leaf">
              {article.title}
            </Link>
          </h2>
          <p className="mt-3 text-lg font-medium leading-snug text-betera-ink/70 md:text-xl">{deck}</p>
    ...
    >
      {img && !compact ? (
        <Link
          ...
          className="relative block h-28 w-full shrink-0 overflow-hidden rounded-xl border border-betera-sand sm:h-24 sm:w-40"
        >
          <UiImage
            src={article.imageUrl}
            alt={article.title}
      ...
      <div className="min-w-0 flex-1">
        <div className="mb-1.5 flex flex-wrap items-center gap-2 text-xs text-betera-ink/55">
          <CategoryChip category={article.category} />
          <time dateTime={article.createdAt.toISOString()}>{formatDate(article.createdAt)}</time>
        </div>
        <h3 className="font-serif text-lg font-semibold leading-snug text-betera-ink md:text-xl">
          <Link href={`/noticias/${article.slug}`} className="transition hover:text-betera-leaf">
            {article.title}
          </Link>
        </h3>
        <p
          className={`mt-2 text-sm leading-relaxed text-betera-ink/70 md:text-base ${

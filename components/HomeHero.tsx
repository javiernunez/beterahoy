...
export function HomeHero({ isVal }: Props) {
  return (
    ...
    >
      ...
      />
      <div
        className="pointer-events-none absolute -bottom-20 left-1/4 h-48 w-48 rounded-full bg-betera-gold/15 blur-3xl"
        aria-hidden
      />

      <p className="betera-section-kicker !text-betera-gold/90">{isVal ? "Camp de Túria · València" : "Camp de Túria · Valencia"}</p>
      <h1 id="home-hero-title" className="mt-2 max-w-2xl font-serif text-3xl font-bold leading-tight md:text-4xl lg:text-[2.75rem]">
        ...
        <span className="text-betera-gold">Bétera</span>
      </h1>
      <p className="mt-4 max-w-xl text-base leading-relaxed text-betera-cream/85 md:text-lg">
        {isVal
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        <CtaLink
          ...
          className="rounded-xl bg-betera-leaf px-5 py-2.5 text-sm font-bold text-white shadow-md transition hover:bg-betera-leaf-dark"
        >
        ...
        <CtaLink
          ...
          className="rounded-xl border border-betera-cream/30 bg-white/10 px-5 py-2.5 text-sm font-semibold text-white backdrop-blur transition hover:bg-white/20"
        >
      ...
      <ul className="mt-8 flex flex-wrap gap-2 border-t border-white/15 pt-6">
        {QUICK_ACCESS.map((item) => (
          <li key={item.href}>
            <Link
              href={item.href}
              className="inline-block rounded-lg bg-white/10 px-3 py-1.5 text-xs font-semibold text-betera-cream transition hover:bg-betera-leaf/90 hover:text-white"
            >
              {item.label}
            </Link>
          </li>

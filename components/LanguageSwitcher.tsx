...
export function LanguageSwitcher({ locale, returnTo = "/", variant = "default" }: LanguageSwitcherProps) {
  ...
  const shellClass = onDark
    ...
    : "inline-flex items-center gap-1 rounded-lg border border-betera-sand bg-white p-0.5";
  const activeClass = onDark ? "bg-betera-leaf text-white shadow-sm" : "bg-betera-forest text-white shadow-sm";
  const idleClass = onDark ? "text-betera-cream/85 hover:bg-white/10" : "text-betera-ink/70 hover:bg-betera-mist";

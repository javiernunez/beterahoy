...
function formatSegmentTitle(segment: string): string {
  return decodeURIComponent(segment)
    ...
    .filter(Boolean)
    .map((w) => {
      if (w.toLowerCase() === "leliana") return "Bétera";
...
export function BreadcrumbBar({ locale }: Props) {
  ...
  return (
    <div className="border-b border-betera-sand/80 bg-betera-mist/50">
      <div className="container-page py-2.5">
        <nav aria-label="Breadcrumb">
          <ol className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-betera-ink/65">
            {items.map((item, i) => {
              const last = i === items.length - 1;
              return (
                <li key={item.path} className="inline-flex min-w-0 max-w-full items-center gap-x-2">
                  {i > 0 ? (
                    <span aria-hidden className="shrink-0 text-betera-sand">
                      /
                    </span>
                  ) : null}
                  {last ? (
                    <span className="min-w-0 truncate font-semibold text-betera-forest">{item.name}</span>
                  ) : (
